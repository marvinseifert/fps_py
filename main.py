...
if __name__ == '__main__':
    ...
    #queue2 = Queue()
    try:
        arduino_obj = connect_to_arduino()
    except FileNotFoundError:
        arduino_obj = None
    sync_queue = Queue()
    queue_lock = Lock()
    p1 = Process(target=tkinter_app, args=(queue1,queue_lock, arduino_obj)) # Start the GUI
    p2 = Process(target=pyglet_app_lead, args=(config_dict, queue1,sync_queue,queue_lock, arduino_obj)) # Start the pyglet app
